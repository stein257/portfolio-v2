...
const PhoneContent = () => {
  const [logoTransform, setLogoTransform] = useState("scale(0)");
  ...
  return (
    <div
      className="absolute w-[6.5vw] h-[10.5vw] bg-background border border-[0.25vw] border-secondary-stroke rounded-[0.5vw]
            ...
            hover:translate-y-[-2vw] hover:cursor-pointer
            [@media(max-height:600px)]:hover:translate-y-[0] [@media(max-height:600px)]:hover:cursor-pointer"
      onMouseEnter={() => {
        setLogoTransform("scale(1)");
      }}
      onMouseLeave={() => {
        setLogoTransform("scale(0)");
      }}>
      <svg
